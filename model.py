...
def four_in_a_row_diagonal_up_right(board):
    ...
    for row in range(3):
        for col in range(4):
            window = board[row:row+4,col:col+4]
            diag = np.diagonal(np.fliplr(window))
            if diag[0] != 0 and np.all(diag==diag[0]):
                return diag[0]
    return 0
...
def check_winner(board):
    ...
    for check in (four_in_a_row_diagonal_down_right,
...
def encode_board(board, current_player):
    ...
    encoded_plane = np.stack([current_plane,opponent_plane])
    return encoded_plane
...
def board_to_torch_tensor(board, current_player):
    ...
    tensor = torch.from_numpy(encoded_board)
...
def masked_log_softmax(logits, mask):
    ...
    logits_clone = masked_policy_logits(logits,mask)
    return F.log_softmax(logits_clone,dim=-1)
...
def ucb_score(parent, child, c_puct=1.5):
    ...
    mean_q = node_q_value(child)
    return mean_q + c_puct * child["prior"] * math.sqrt(parent["visit_count"])/(1+child["visit_count"])
...
def select_best_child(node, legal_actions, c_puct=1.5):
    ...
    for action in legal_actions:
        ...
        curr_score = ucb_score(node,child,c_puct)
...
def play_self_play_game(net, num_simulations, c_puct, temperature=1.0):
    ...
    while not done:
        action , pi = mcts_choose_action(
            board,current_player,net,num_simulations=num_simulations,
